...
class EKF(object):
    ...
    def measurement_update(self, rawZ, rawR):
        ...
        #### TODO ####
        # update self.x, self.P
        sigma = R + (np.matmul(np.matmul(H, self.P), H.T))
...
class Localization_EKF(EKF):
    ...
    def map_line_to_predicted_measurement(self, m):
        ...
        Transformation_robot_to_world = np.array([[np.cos(theta), -np.sin(theta), x],[np.sin(theta),  np.cos(theta), y], [0., 0., 1.]])
        x_camera_world, y_camera_world, _ = Transformation_robot_to_world.dot(np.array([x_camera, y_camera, 1.]))
        h = np.array([alpha - theta - theta_camera, r - x_camera_world*np.cos(alpha) - y_camera_world*np.sin(alpha)])
        Hx = np.array([[0., 0., -1.], [-np.cos(alpha), -np.sin(alpha), (y_camera*np.cos(alpha)-x_camera*np.sin(alpha))*np.cos(theta) + (x_camera*np.cos(alpha)+y_camera*np.sin(alpha))*np.sin(theta)]])
        ##############
    ...
    def associate_measurements(self, rawZ, rawR):
        ...
        # Declaring empty arrays for lists
        v_list, R_list, H_list = [], [], []
        num_measurements = rawZ.shape[1]
